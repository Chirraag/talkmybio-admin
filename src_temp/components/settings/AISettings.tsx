...
export const AISettings: React.FC = () => {
  ...
  // Fetch voices and admin preferences
  useEffect(() => {
    const fetchData = async () => {
      if (!auth.currentUser) return;

      try {
        // Initialize a new Retell client for fetching voices
        ...
        // Fetch admin preferences from Firestore
  ...
  // Stop any currently playing audio when the voice changes
  useEffect(() => {
    if (audioRef.current) {
  ...
  const updateRetellAgents = async (usersSnapshot: any) => {
    ...
    if (failedUpdates.length > 0) {
      throw new Error(`Failed to update some agents: ${failedUpdates.join(', ')}`);
    }
  };

  // Save changes by updating Firestore and then calling the Retell API for each agent update
